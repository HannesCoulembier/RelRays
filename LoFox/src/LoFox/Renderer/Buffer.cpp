...
namespace LoFox {
	...
	Buffer::Buffer(Ref<RenderContext> context, uint32_t size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
		...
		LF_CORE_ASSERT(vkCreateBuffer(m_Context->LogicalDevice, &bufferInfo, nullptr, &m_Buffer) == VK_SUCCESS, "Failed to create buffer!");
		...
		memoryAllocInfo.memoryTypeIndex = Utils::FindMemoryType(m_Context->PhysicalDevice, memRequirements.memoryTypeBits, properties);

		LF_CORE_ASSERT(vkAllocateMemory(m_Context->LogicalDevice, &memoryAllocInfo, nullptr, &m_Memory) == VK_SUCCESS, "Failed to allocate buffer memory!");
	...
	void Buffer::SetData(const void* src) {
		...
		void* data;
		vkMapMemory(m_Context->LogicalDevice, m_Memory, 0, m_Size, 0, &data);
