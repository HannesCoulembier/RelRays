...
namespace LoFox {
	...
	Ref<Buffer> Buffer::Create(uint32_t size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties) {

		return CreateRef<Buffer>(size, usage, properties);
	}
	...
	void Buffer::CopyBuffer(Ref<Buffer> srcBuffer, Ref<Buffer> dstBuffer) {
		...
		VkCommandBuffer commandBuffer = VulkanContext::BeginSingleTimeCommandBuffer();
		...
		VulkanContext::EndSingleTimeCommandBuffer(commandBuffer);
	}
}